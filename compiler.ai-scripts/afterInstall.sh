...
if [ ! -f "$ROOT" ]; then
	$SUDO -u compilerai-server mkdir -p $PARENTDIR
	$SUDO -u compilerai-server git -C $PARENTDIR clone https://compilerai-bot:SaouK7or7nJwUBIRDNF9@github.com/bsorav/superopt-project
	$SUDO -u compilerai-server git -C $ROOT checkout $BRANCH
fi
...
if [ ! -f "/usr/bin/node" ]; then
	$SUDO -u compilerai-server bash -c "mkdir -p $ROOT/compiler.ai-scripts/build && cd $ROOT/compiler.ai-scripts/build && git clone https://github.com/nodejs/node && cd - && cd $ROOT/compiler.ai-scripts/build/node && git checkout v13.x && ./configure && make && cd -" && make -C $ROOT/compiler.ai-scripts/build/node install PREFIX=/usr && setcap cap_net_bind_service=+eip /usr/bin/node #this is to allow listening on port 80)
fi
$SUDO -u compilerai-server bash -c "cd $ROOT/compiler.ai-scripts/compiler-explorer && npm update && npm install webpack-dev-server --save-dev && touch .npm-updated && cd -"
$SUDO -u compilerai-server ln -sf `pwd`/tars $ROOT/tars
$SUDO -u compilerai-server make -C $ROOT build SUPEROPT_INSTALL_DIR=/usr/local
make -C $ROOT install SUPEROPT_INSTALL_DIR=/usr/local
$SUDO -u compilerai-server make -C $ROOT compiler_explorer_preload_files
